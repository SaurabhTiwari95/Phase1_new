...
<div class="container" style="margin-left: 180px;margin-top: 20px">
    <a class="btn btn-default" href="admin_login.php">Go Back to Login</a>
    ...
    <form method="post" action="">
        <div class="container">
            ...
            if (isset($_POST['sub2'])) {
                ...
                $q = mysqli_query($db, "select * from admin where amobile ='$mobile'");
            if (mysqli_num_rows($q) > 0) {
                ...
                $nk = mysqli_query($db, "insert into admin_otp(otp_contact,otp_mail,otp_userid)values('$mob1','$mail','$uid')");

                $upd = mysqli_query($db, "update admin_otp set otp_no='$s' where otp_contact='$mobile'");
            if ($upd) {
            ...
            $otp1 = $_POST['votp'];
            $nn = mysqli_query($db, "select * from admin_otp where otp_no='$otp1'");
            if (mysqli_num_rows($nn) == 1) {
            $a = mysqli_fetch_assoc($nn);
            $_SESSION['cont1'] = $a['otp_contact'];

            ?>
                <div><h1 style="color: white">otp verification successful</h1>

                    <h2 style="color: white">now change the password</h2>

                    <a class="btn btn-default" href="admin_passchange.php">click me to change password</a>
            ...
            {
            $q = mysqli_query($db, "select * from admin where aemail='$email'");
            if (mysqli_num_rows($q) > 0) {
            $row = mysqli_fetch_assoc($q);
            $mob1 = $row['amobile'];
            $uid = $row['aid'];
            ...
            $upd = mysqli_query($db, "update admin_otp set otp_no='$s' where otp_mail='$email'");
            if ($upd)
            {
            ?>
                <script>
